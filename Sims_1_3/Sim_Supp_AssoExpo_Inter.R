...
AgeMinCens            <- c(0, 20)
AgeMaxCens            <- c(30, 50)
ALLPRMTR0             <- expand.grid(R2BMI, ALLlogHRBMI, ALLlogHRSNP, ALLlogHRMatching, ALLMAF, PSELCASES, AgeMinCens, AgeMaxCens)
colnames(ALLPRMTR0)   <- c("R2BMI", "logHRBMI", "logHRSNP", "logHRMatching", "MAF", "PSELCASES", "AgeMinCens", "AgeMaxCens")
...
DirError      <- "Res/"
suffix <- paste0("Sim_Supp_Asso_Inter_CohortSize_", CohortSize, "_MAF_", MAF, "_R2BMI_", R2BMI, "_logHRSNP_", round(logHRSNP,2),
...
tryCatch({
  tempcohort_sim1 <-  simul_cohort_interMatching(n=CohortSize, R2BMI, logHRSNP, logHRBMI, logHRMatching,
...
internal_task <- function(nsim) {
  ...
  tryCatch(
    {
      ...
      {
        tempcohort <-  simul_cohort_interMatching(n=CohortSize, R2BMI, logHRSNP, logHRBMI, logHRMatching,
                                                  MeanBMI=NA, SDBMI=NA, MeanSNP=NA, SDSNP=NA,
                                                  MAF=MAF, agemincens, agemaxcens)
      }
    },
  ...
  tryCatch(
    {
      monster <-  c(nsim, pselCases, MAF, R2BMI, logHRSNP, logHRBMI, logHRMatching, 
...
allnames <- c("nsim", "pselCases","MAF","R2BMI", "logHRSNP", "logHRBMI", "logHRM", "Agemincens", "Agemaxcens", "AssoMsnp",
