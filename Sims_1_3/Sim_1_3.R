...
ALLPRMTR_sim1         <- rbind(expand.grid(ALLR2SNPBMI, ALLlogHRBMI, ALLlogHRSNP, log(2), ALLMAF, PSELCASES, AgeMinCens, AgeMaxCens, NNMatching, 1), 
                               c(ALLR2SNPBMI, ALLlogHRBMI, ALLlogHRSNP, log(1), ALLMAF, PSELCASES, AgeMinCens, AgeMaxCens, 1, 0))
ALLPRMTR_sim3         <- rbind(c(ALLR2SNPBMI, ALLlogHRBMI, ALLlogHRSNP, log(2), ALLMAF, PSELCASES, AgeMinCens, AgeMaxCens, 0, 1),
                               c(ALLR2SNPBMI, ALLlogHRBMI, ALLlogHRSNP, log(2), ALLMAF, PSELCASES, AgeMinCens, AgeMaxCens, 0, 0),
                               c(ALLR2SNPBMI, ALLlogHRBMI, ALLlogHRSNP, log(1), ALLMAF, PSELCASES, AgeMinCens, AgeMaxCens, 0, 0))

colnames(ALLPRMTR_sim1) <- colnames(ALLPRMTR_sim3)    <- c("R2SNPBMI", "logHRBMI", "logHRSNP", "logHRMatching", "MAF", "PSELCASES", "AgeMinCens", "AgeMaxCens", "NNmatching", "Asso_M_SNP")

ALLPRMTR              <- rbind(ALLPRMTR_sim1, ALLPRMTR_sim3[-1, ]) # because the first row of ALLPRMTR_sim3 is already in ALLPRMTR_sim1 :)
...
DirError      <- "Res/"
suffix <- paste0("Sim_1_3_CohortSize_", CohortSize, "_MAF_", MAF, "_R2SNPBMI_", R2SNPBMI, "_logHRSNP_", round(logHRSNP,2),
...
internal_task <- function(nsim) {
  ...
  tryCatch({
    ...
    # if(typesim=="Sim3")
    {
      mod_casectrlWeightedKM_nomatching         <- lm(BMI ~ SNP, weights= WeightsKM_noMatching, data= cohort_withweights)
    ...
    modCox_casectrlWeightedKM_nomatching     <- coxph(Surv(AgeOut, Ind) ~ SNP + BMI + Matching, weights= WeightsKM_noMatching, data = subset(cohort_withweights, statforweight_02>0))
    ...
    KM_casectrlWeightedKM_nomatching          <- 1 - summary(km_est$fitKM[5], times = agemaxcens-10, extend=T)$surv
    ...
    KM_cond_casectrlWeightedKM_nomatching     <- 1 - summary(km_cond_est$fitKM[5], times = agemaxcens-10, extend=T)$surv
  },
  ...
  tryCatch(
    {
        monster <-  c(nsim, pselCases, MAF, R2SNPBMI, logHRSNP, logHRBMI, logHRMatching,assoMsnp_num, 
                    ...
                    coef(mod_casectrlWeightedKM_nomatching)[2],
