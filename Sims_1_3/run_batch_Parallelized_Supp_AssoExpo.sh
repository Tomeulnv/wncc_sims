...
#SBATCH --array=0-8 # EDIT set len(array) as len(parameter combo grid)
#SBATCH -p low_p
...
Rscript Sim_Supp_AssoExpo_Inter.R "$SLURM_ARRAY_TASK_ID" "$WORKDIR"
Rscript Sim_Supp_AssoExpo.R "$SLURM_ARRAY_TASK_ID" "$WORKDIR"
